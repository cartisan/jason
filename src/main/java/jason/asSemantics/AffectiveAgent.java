...
public class AffectiveAgent extends Agent {
    ...
    @Override
    public void initAg() {
        this.ts = new AffectiveTransitionSystem(this.ts);  // the TransitionSystem sets this.ts to itself in its own init
        super.initAg();
    ...
    public AffectiveAgent clone(AgArch arch) {
        ...
        try {
            ...
            this.logger.severe(" cannot create derived class" + e1);
            return null;
        } catch (IllegalAccessException e2) {
            this.logger.severe(" cannot create derived class" + e2);
            return null;
        }
        ...
        synchronized (this.getBB().getLock()) {
            a.bb = this.bb.clone();
        }
        a.pl = this.pl.clone();
        try {
            this.fixAgInIAandFunctions(a);
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        switch(condition.getFunctor()) {
            ...
            case AffectiveDimensionChecks.NOT: {
                ...
                if(!(innerCondition.size()==1)) {
                    ...
                    this.getLogger().severe("ERROR: Plan annotation " + condition.toString() + " is malformed, too many terms.");
            ...
            default: {
                this.logger.severe("*** ERROR in AffectiveAgent::checkConstraint >> Plan annotation " +
                              condition.toString() +
                              " has invalid functor.");
    ...
    protected void updateMoodTarget() throws RevisionFailedException {
        if (this.targetsChanged) {
            ...
            List<Term> targets = this.getAffectiveTS().getAffectiveC().T.stream().map(ASSyntax::createAtom).collect(Collectors.toList());
