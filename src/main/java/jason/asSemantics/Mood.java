...
public class Mood implements Serializable, Affect {
    ...
    private static int MAX_UPDATE_TIME = 5;
    ...
    public void updateMood(List<Emotion> emotions, Personality personality) {
        ...
        List<Double> step = new LinkedList<>();
        ...
        // N /in [-1,1] | neurot_factor_func: (-1) -> 0.5; 1 -> 1.5; 0 -> 1 | neurot_factor = 1 + 0.5N
    ...
    public String toString() {
        return String.format("(%.4f, %.4f, %.4f) ", this.PAD.getX(), this.PAD.getY(), this.PAD.getZ()) + this.getFullName();
    }
    ...
    public String getFullName() {
        return this.getStrength() + " " + this.getType();
    }
    ...
    public boolean checkConstraint(Literal constraint) {
        ...
        if(constraint.getArity() != 2) {
            this.logger.severe("*** ERROR: Mood annotation " + constraint.toString() + " has wrong arity. Should be 2.");
            return false;
        }
        ...
        if (!DIMENSIONS.contains(dimension)) {
            this.logger.severe("*** ERROR: Mood annotation: " + constraint.toString() + " uses an illegal dimension name");
            return false;
        }
        if (!AffectiveDimensionChecks.BOUNDARIES.containsKey(bound)) {
            this.logger.severe("*** ERROR: Mood annotation: " + constraint.toString() + " uses an illegal trait boundary");
            return false;
        }
